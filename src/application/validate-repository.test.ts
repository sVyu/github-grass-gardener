...
const valid: RepositoryRef = {
  owner: 'octocat', name: 'grass-log', fullName: 'octocat/grass-log', defaultBranch: 'main',
...
describe('repository preflight', () => {
  ...
  it('warns about forks and blocks unsafe write targets', () => {
    const result = validateRepository({ ...valid, isFork: true, isProtected: true, hasPushAccess: false });
    expect(result.canPublish).toBe(false);
    expect(result.warnings).toContain('Fork commits do not count toward GitHub contributions.');
    expect(result.blockers).toEqual(expect.arrayContaining(['No push access to this repository.', 'The default branch is protected.']));
  });
});
