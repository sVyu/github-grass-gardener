...
describe('GitHub API adapter', () => {
  it('loads identity, verified email, and normalized repositories', async () => {
    server.use(
      http.get('https://api.github.com/user', () => HttpResponse.json({ login: 'octocat', id: 7, name: 'Octocat', avatar_url: 'https://avatars.githubusercontent.com/u/7' })),
      ...
      http.get('https://api.github.com/user/repos', () => HttpResponse.json([
        { owner: { login: 'octocat' }, name: 'grass', full_name: 'octocat/grass', default_branch: 'main', private: true, fork: false, archived: false, disabled: false, permissions: { push: true } },
      ])),
    );
    ...
    expect(await client.listRepositories()).toMatchObject([{ fullName: 'octocat/grass', hasPushAccess: true }]);
  });

  it('falls back to the account noreply address when email scope is unavailable', async () => {
    server.use(http.get('https://api.github.com/user/emails', () => HttpResponse.json({ message: 'Resource not accessible' }, { status: 403 })));
    const client = new GitHubClient('test-token');
    expect(await client.getVerifiedEmails({ login: 'octocat', id: 7, name: 'Octocat', avatarUrl: '' })).toEqual([
  ...
  it('normalizes the contribution calendar returned by GraphQL', async () => {
    server.use(http.post('https://api.github.com/graphql', () => HttpResponse.json({
      data: { user: { contributionsCollection: { contributionCalendar: {
        ...
        weeks: [{ firstDay: '2026-01-04', contributionDays: [{ date: '2026-01-04', weekday: 0, contributionCount: 2, contributionLevel: 'SECOND_QUARTILE', color: '#123456' }] }],
    ...
    expect(calendar.weeks[0]?.contributionDays[0]).toMatchObject({ date: '2026-01-04', contributionCount: 2 });
  });

  it('uses a non-forced ref update and sanitizes API failures', async () => {
    let body: unknown;
    server.use(http.patch('https://api.github.com/repos/octocat/grass/git/refs/heads%2Fmain', async ({ request }) => {
