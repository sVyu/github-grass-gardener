...
### 4.3 후속 고도화 후보: 여러 커밋을 한 번에 공개

- **현재 방식**: 커밋 객체를 만들 때마다 ref를 갱신하므로 성공 SHA를 즉시 확정하고, 브라우저가 중단돼도 이전 성공분은 기본 브랜치에 남는다.
- **전체 일괄 방식**: 부모로 연결된 모든 커밋 객체를 만든 뒤 마지막 SHA로 ref를 한 번만 갱신한다. ref 갱신 전 중단되면 객체가 생성됐더라도 기본 브랜치에는 어떤 새 커밋도 공개되지 않는다.
- **청크 방식**: 여러 커밋을 한 묶음으로 생성한 뒤 묶음마다 ref를 한 번 갱신한다. ref 갱신 횟수와 중단 시 미발행 범위 사이의 절충안이며, 묶음 크기는 아직 결정하지 않았다.
- 어떤 방식을 택하든 Blob·Tree·Commit 생성 요청은 커밋별로 필요하다. ref 갱신 횟수 감소가 API 속도 제한 회피나 기여 캘린더의 빠른 반영을 보장하지 않는다. HEAD 재검사, `force: false`, 확인된 ref 갱신 후 성공 기록, 재접속 시 원격 상태 대조를 유지한다.
